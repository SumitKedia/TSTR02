...
log_and_execute ./gradlew --no-daemon -I $OUTPUT_DIR/init.gradle cyclonedxBom -PbomOutputDir=$OUTPUT_DIR/sbom
...
GRADLEW_VERSION=$(./gradlew --version | grep 'Gradle' | awk '{print $2}')
APP_META_DATA=",\"appmetadata\":[{\"key\":\"language\", \"value\":\"java\"},{\"key\":\"javaversion\", \"value\":\"${JAVA_VERSION}\"},{\"key\":\"gradleversion\", \"value\":\"${GRADLEW_VERSION}\"}]"
...
for file in "$directory"/*; do
    ...
    if [ -f "$file" ]; then
        ...
        echo "$APP_META_DATA" >> "$output_file"
